...
USE CompanySupplyProduct SELECT *FROM Company
USE CompanySupplyProduct SELECT *FROM Product
USE CompanySupplyProduct SELECT *FROM Supply
...
SELECT COUNT(CompanyID) AS [So luong],Address FROM Company
GROUP BY Address

SELECT Company.Name
FROM Company
WHERE EXISTS(SELECT ProductID FROM Supply WHERE Company.CompanyID = Supply.CompanyID)

SELECT DISTINCT Name
FROM Company, Supply
WHERE Company.CompanyID = Supply.CompanyID

--cho biet sp co gia lon nhat

SELECT *FROM Product
WHERE Price <= All(SELECT 300 FROM Product)
